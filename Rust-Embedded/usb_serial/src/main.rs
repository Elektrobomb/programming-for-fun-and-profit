...
use embassy_stm32::{bind_interrupts, peripherals, Config, usb, Peripherals};
...
async fn main(_spawner: Spawner) {
    ...
    let mut p: Peripherals = embassy_stm32::init(config);
    // Spawn the LED task.
    ...
    config_and_start_usb_serial(&mut p.USB_OTG_FS, &mut p.PA12, &mut p.PA11).await.unwrap();
}
...
async fn config_and_start_usb_serial<Dp, Dm>(
    usb_otg_fs: &mut peripherals::USB_OTG_FS,
    dp: &mut Dp,
    dm: &mut Dm,
) -> Result<(), ()>
where
    // This allows the function to accept any pin that implements the usb::DpPin and usb::DmPin traits.
    ...
    let usb_driver = Driver::new_fs(usb_otg_fs, Irqs, dp, dm, &mut ep_out_buffer, config);

    // Create embassy-usb Config
    let mut usb_config = embassy_usb::Config::new(0xc0de, 0xcafe);
    usb_config.manufacturer = Some("usb-serial example");
